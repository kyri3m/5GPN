...
def validate_host_port(host, port):
    if not isinstance(port, int) or not 1 <= port <= 65535:
        die("invalid upstream port")
    try:
        ipaddress.ip_address(host)
        return
    except ValueError:
        pass
    if len(host or "") > 253 or not re.fullmatch(
        r"(?=.{1,253}\Z)(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?\.)*"
        r"[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?",
        host or "",
    ):
        die("invalid upstream hostname")
...
def parse_socks(uri):
    ...
    hostport = re.split(r"[/?#]", hostport, maxsplit=1)[0].strip()
...
def main():
    ...
    if remote_dns:
        tun["dns-hijack"] = []
    proxy = {"name": name, "type": ptype, **proxy}
    config = {
        "mode": "rule",
        "log-level": "warning",
        "dns": {"enable": False},
    ...
    sys.stdout.write("# Mihomo exit config: %s (%s)\n" % (name, ptype))
